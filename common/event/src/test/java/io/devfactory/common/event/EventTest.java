package io.devfactory.common.event;

import io.devfactory.common.event.payload.ArticleCreatedEventPayload;
import lombok.extern.slf4j.Slf4j;
import org.junit.jupiter.api.Test;

import java.time.LocalDateTime;

import static org.assertj.core.api.Assertions.assertThat;

@Slf4j
class EventTest {

  @Test
  void SerializationTest() {
    // given
    final var payload = ArticleCreatedEventPayload.builder()
      .articleId(1L)
      .title("title")
      .content("content")
      .boardId(1L)
      .writerId(1L)
      .createdAt(LocalDateTime.now())
      .modifiedAt(LocalDateTime.now())
      .boardArticleCount(23L)
      .build();

    final var event = Event.of(1234L, EventType.ARTICLE_CREATED, payload);
    final var json = event.toJson();
    log.info("[dev] json = {}", json);

    // when
    final var result = Event.fromJson(json);

    // then
    assertThat(result.getEventId()).isEqualTo(event.getEventId());
    assertThat(result.getEventType()).isEqualTo(event.getEventType());
    assertThat(result.getPayload()).isInstanceOf(payload.getClass());

    final var resultPayload = (ArticleCreatedEventPayload) result.getPayload();
    assertThat(resultPayload.getArticleId()).isEqualTo(payload.getArticleId());
    assertThat(resultPayload.getTitle()).isEqualTo(payload.getTitle());
    assertThat(resultPayload.getCreatedAt()).isEqualTo(payload.getCreatedAt());
  }

}
